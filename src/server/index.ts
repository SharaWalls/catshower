...
router.post('/submit-score', async (req, res): Promise<void> => {
  try {
    console.log('[API] Submit score API called with body:', req.body);
    ...
    if (!playerScore.playerId || !playerScore.playerName || typeof playerScore.enduranceDuration !== 'number') {
      ...
      console.error('[API] Submit score validation error:', errorMsg);
      res.status(400).json({ 
        status: 'error', 
        message: errorMsg
      });
    ...
    console.log(`[API] Processing score submission: ${playerScore.playerName} (${playerScore.playerId})`);
    console.log(`[API] Endurance duration: ${playerScore.enduranceDuration}s, Continent: ${playerScore.continentId}`);
    ...
    console.log('[API] Score submission result:', result);
...
router.get('/leaderboard', async (req, res): Promise<void> => {
  try {
    console.log('[API] Leaderboard API called with query:', req.query);
    
    const limit = parseInt(req.query.limit as string) || 100;
    const continentId = req.query.continentId as string;
    const { redis } = getContext();

    console.log(`[API] Getting leaderboard with limit: ${limit}, continentId: ${continentId || 'global'}`);
    ...
    console.log('[API] Leaderboard data retrieved:', {
    ...
    console.log('[API] 准备发送排行榜数据:', leaderboardData);
...
router.get('/leaderboard/stats', async (_req, res): Promise<void> => {
  try {
    console.log('[API] Continent stats API called');
    
    const { redis } = getContext();
    const continentStats: ContinentStats[] = await getContinentStats({ redis });

    console.log('[API] Continent statistics retrieved:', continentStats);
    console.log('[API] 准备发送洲际统计数据:', continentStats);
...
router.get('/player-best', async (req, res): Promise<void> => {
  try {
    console.log('[API] Player best API called with query:', req.query);
    ...
    console.log(`[API] Getting best score for player: ${playerId}`);

    const playerBest = await getPlayerBest({ redis, playerId });

    console.log('[API] Player best score retrieved:', playerBest);
...
router.post('/add-test-data', async (_req, res): Promise<void> => {
  try {
    ...
    console.log('[API] Adding test data to leaderboard...');
    ...
    for (const player of testPlayers) {
      try {
        ...
        results.push(result);
        console.log(`[API] Added test player: ${player.playerName} - ${player.enduranceDuration}s`);
      } catch (error) {
        console.error(`[API] Failed to add test player ${player.playerName}:`, error);
      }
    }

    console.log(`[API] Test data addition completed. Added ${results.length} players.`);
    ...
    console.error('[API] Add test data error:', error);
    res.status(500).json({
      status: 'error',
